  ...
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      output$samlerapport <- shiny::renderUI({
        rapbase::renderRmd(
          system.file("samlerapport.Rmd", package = "barnehofte"),
      ...
      output$downloadSamlerapport <- shiny::downloadHandler(
        filename = function() {
          basename(tempfile(pattern = "barnehofteSamlerapport",
                            fileext = paste0(".", input$formatS)))
        },
        content = function(file) {
          srcFile <-
            normalizePath(system.file("samlerapport.Rmd", package = "barnehofte"))
